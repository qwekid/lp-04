...
### Поддерживаемые флаги для uname
 ...
 ## Как использовать cp
Требуется запустить исполняемый файл cp
```
./cp [-r] [-i] [--help] <исходный путь> <конечный путь>
```
### Поддерживаемые флаги для cp
  -r             Рекурсивное копирование директорий.
  -i             Запрос на перезапись файла, если файл существует.
  --help         Показать справку.

 ## Как использовать history
Требуется запустить исполняемый файл history
```
./history [-n <число>] [-с] [--help]
```
### Поддерживаемые флаги для history
    -n x - Показать последние x команд.
  	-c - Очистить историю команд.
  	--help - Показать справку.

## Как использовать pwgen
Требуется запустить исполняемый файл pwgen
```
./pwgen [-length <число>] [-s] [-n] [-c] [--help]
```
### Поддерживаемые флаги для pwgen
  	-length - Длина пароля (по умолчанию 8 символов).
  	-s - Генерировать сильный пароль (смешанные символы).
  	-n - Генерировать числовой пароль.
  	-c - Генерировать пароль с заглавной буквы.
  	--help - Показать справку.

## Как использовать hexdump
Требуется запустить исполняемый файл hexdump
```
./hexdump [-file <путь/к/файлу>] [-n] [-C] [--help]
```
### Поддерживаемые флаги для hexdump
  	-file <путь> - Путь к файлу для дампа.
  	-n <число> - Число байтов для вывода (по умолчанию все данные).
  	-C - Показывать ASCII-эквивалент данных.
  	--help - Показать справку.

## Как использовать find
Требуется запустить исполняемый файл find
```
./find [-path <путь к дирректории поиска>] [-name <иия файла>] [-type <тип файла>] [-maxdepth <глубина поиска>] [--help]
```
### Поддерживаемые флаги для find
  -path <путь>    Путь к директории для поиска (по умолчанию текущая).
  -name <имя>     Поиск по имени файла.
  -type <тип>     Поиск по типу файла (f — обычный файл, d — директория).
  -maxdepth <глубина>  Ограничение глубины поиска.
  --help          Показать справку.
