...
func main() {
	...
	for i := 1; i < len(os.Args); i++ {
		...
		case arg == "-h":
			fmt.Println("Использование: ./mkdir [-p] [-v] [-m <права доступа>] <путь>)")
			fmt.Println("  -p: Создает промежуточные каталоги, если они не существуют")
			fmt.Println("  -v: Включает режим подробного вывода, который сообщает о создании директории")
			fmt.Println("  -m: задать права доступа для создаваемой директории в восьмеричном формате")
			return
		default:
